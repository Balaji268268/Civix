    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // Try Google Maps Geocoding Service (Premium & Reliable)
          let location = '';
          let success = false;
          let serviceUsed = '';
          ...
          if (window.google && window.google.maps) {
            try {
              ...
              if (response.results && response.results[0]) {
                console.log('Google Maps Geocoding response:', response.results[0]);
                ...
                const adminArea = getComponent('administrative_area_level_1'); // State
                ...
                if (locality && adminArea) {
                  ...
                  location = `${adminArea}, ${country}`;
                } else {
                  location = response.results[0].formatted_address; // Fallback to full address
                }

                success = true;
                serviceUsed = 'Google Maps API';
              }
            } catch (gError) {
              console.error('Google Maps Geocoding failed:', gError);
            }
          }

          // Fallback: If Google Maps fails or isn't loaded, use OpenStreetMap (No API Key req)
          if (!success) {
            try {
