...
const Analytics = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50/30 to-emerald-50/50 ml-0 lg:ml-20 transition-all duration-300">
      {isSidebarOpen && (
      ...
      <div className="px-4 py-8 sm:px-6 lg:px-8">
        ...
        {/* --- AI INSIGHTS PANEL (Genius Section) --- */}
        {stats?.aiInsights && stats.aiInsights.length > 0 && (
          <div className="mb-10 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-950/20 dark:to-purple-900/20 border border-indigo-100 rounded-2xl p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-indigo-100 rounded-lg text-indigo-600">
                <BarChart3 className="w-6 h-6" />
              </div>
              <h3 className="text-xl font-bold text-indigo-900 dark:text-indigo-200">AI Genius Insights</h3>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {stats.aiInsights.map((insight, idx) => (
                <div key={idx} className="bg-white/80 dark:bg-gray-800/80 p-4 rounded-xl border border-indigo-50 shadow-sm flex items-start gap-3">
                  <span className="text-2xl mt-1">{insight.substring(0, 2)}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-2 rounded-2xl bg-white/80 backdrop-blur-sm border border-green-100/50 shadow-xl shadow-green-100/20 p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
              <h3 className="text-xl font-bold text-slate-800">Issue Trends (Last 7 Days)</h3>
              ...
              {categoryData.map((category, index) => (
                <div key={index} className="flex items-center justify-between p-2 rounded-lg hover:bg-emerald-50/50 transition-colors">
                  <div className="flex items-center">
                    ...
                    <span className="text-sm text-slate-600 font-medium truncate max-w-[120px]" title={category.name}>{category.name}</span>
                  </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          <div className="rounded-2xl bg-white/80 backdrop-blur-sm border border-green-100/50 shadow-xl shadow-green-100/20 p-6 hover:shadow-2xl hover:shadow-green-200/30 transition-all duration-300">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500">Contact Queries</p>
                <p className="text-2xl font-bold text-slate-800">{stats?.contact?.pending || 0}</p>
                <p className="text-sm text-emerald-600 font-medium">Pending Reply</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-white/80 backdrop-blur-sm border border-green-100/50 shadow-xl shadow-green-100/20 p-6 hover:shadow-2xl hover:shadow-green-200/30 transition-all duration-300">
            <div className="flex items-center">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500">Lost & Found</p>
                <p className="text-2xl font-bold text-slate-800">{stats?.lostFound?.lost || 0}</p>
                <p className="text-sm text-emerald-600 font-medium">{stats?.lostFound?.found || 0} items found</p>
              </div>
            </div>
          </div>
