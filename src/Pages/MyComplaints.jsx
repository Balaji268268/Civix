...
const MyComplaints = () => {
  ...
  if (loading) {
    return <div className="min-h-screen flex items-center justify-center text-emerald-600">Loading complaints...</div>;
  }
        ...
        <div className="max-w-5xl mx-auto pt-20">
          ...
          {filteredComplaints.length === 0 ? (
            <div className="text-center py-24">
              ...
              <p className="text-emerald-500/80 text-lg">Try selecting a different filter or <Link to="/report-issue" className="underline font-bold">report a new issue</Link>.</p>
            </div>
          ) : (
            <div className="grid gap-7">
              {filteredComplaints.map((issue) => (
                <Link to={`/issues/${issue._id}`}
                  key={issue._id}
                  className="group relative bg-white/80 dark:bg-emerald-950/80 border border-emerald-200/40 dark:border-emerald-800/50 shadow-xl hover:shadow-2xl rounded-3xl p-8 transition duration-300 hover:-translate-y-1 block"
                >
                  ...
                  <div className="relative z-10">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                      <div className="flex items-center gap-3 text-emerald-600 dark:text-emerald-300">
                        ...
                        <span className={`px-3 py-1 text-xs rounded-full font-medium border ${issue.issueType === 'Public' ? 'bg-blue-50 text-blue-600 border-blue-100' : 'bg-purple-50 text-purple-600 border-purple-100'}`}>
                    ...
                    <p className="text-gray-800 dark:text-emerald-100 text-xl leading-relaxed mb-4 font-medium line-clamp-2">{issue.title}</p>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-5 mt-2 border-t border-emerald-100/40 dark:border-emerald-700/40 gap-4">
                      ...
                      {/* Priority Tag (ML) */}
                      {issue.priority && (
                        <div className={`px-3 py-1 rounded-lg text-sm font-bold border ${issue.priority === 'High' ? 'bg-red-100 text-red-700 border-red-300' :
                          issue.priority === 'Medium' ? 'bg-orange-100 text-orange-700 border-orange-300' :
                      ...
                      <div className="flex items-center gap-2 text-emerald-500 font-semibold">
                        ...
                        <span className="text-base font-medium text-emerald-600 dark:text-emerald-400">ID: {issue._id.slice(-6).toUpperCase()}</span>
                      </div>
                    </div>
                  </div>
                </Link>
