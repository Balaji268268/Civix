...
const App = () => {
  ...
  const [showGuide, setShowGuide] = useState(false); // Guide State

  useEffect(() => {
    // Show guide only if signed in and NOT seen before
    if (isSignedIn && !localStorage.getItem("hasSeenGuide")) {
      setShowGuide(true);
      // Also show toast once
      // toast.success("Signed in successfully! Welcome to Civix.");
    }
  }, [isSignedIn]);
